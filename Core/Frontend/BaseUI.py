...
class BaseUI:
    ...
    def listenKeyInput(self):
        if os.name == "nt":
            try:
                import msvcrt
            except:
                self.myLogger.log("W", "Failed to import msvcrt, some functions may fail to work.")
        else:
            try:
                import tty
                import termios
            except:
                self.myLogger.log("W", "Failed to import tty or termios, some functions may fail to work.")
        if os.name == "nt":
            ch = msvcrt.getch()
            if ch == b'\xe0':
                second = msvcrt.getch()
                ...
                return key_map.get(second, None)
            elif ch == b'\r':
                return 'ENTER'
            elif ch == b' ':
                return 'SPACE'
            ...
            try:
                ...
                termios.tcgetattr(old_settings)
